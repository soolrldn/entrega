...
/* -----------------------------------------------------
 ...
 const diffMedia = document.querySelector('.diff-media');
  ...
  const diffSoundToggle = diffMedia ? diffMedia.querySelector('.diff-sound-toggle') : null;
  if (diffSoundToggle && diffVideo) {
    diffSoundToggle.addEventListener('click', () => {
      diffVideo.muted = !diffVideo.muted;
      diffSoundToggle.setAttribute('aria-pressed', String(!diffVideo.muted));
      diffSoundToggle.setAttribute(
        'aria-label',
        diffVideo.muted ? 'Activar sonido del video' : 'Silenciar video'
      );
    });
  }
  /* -----------------------------------------------------
...
/* ----------------------------------------------------
  ...
  if (stepper) {
    const stepsData = [
      { title: "Sabés dónde estás antes de empezar.", text: "Una sesión de evaluación completa con informe técnico individualizado. Estilo de manejo y comparativa contra datos de referencia." },
      { title: "Lo que no se mide, no se mejora.", text: "Cada vuelta genera datos. Trabajás con un coach sobre tu propia data para optimizar frenadas, trazadas y sectores críticos con precisión." },
      { title: "La mejora no es aleatoria. Es un plan.", text: "Entrenamientos enfocados en corregir los puntos detectados durante el análisis. Cada sesión tiene objetivos y métricas de progreso." },
      { title: "El campeonato no se improvisa. Se entrena.", text: "Competís en el ecosistema de ligas de Apex con datos registrados y análisis post-carrera. La presión del resultado real es parte del método." }
    ];
    const stepEls = stepper.querySelectorAll('.step');
    ...
    const setStep = (i) => {
      ...
      bodyTitle.textContent = stepsData[i].title;
      bodyText.textContent = stepsData[i].text;
    };
